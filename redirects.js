...
//pageNames.map  create buttons and button events written by Cody Swartz
...
var AutoRedirects = {
    ...
    autoApplyRedirects: function () {
        ...
        //check if the last character is "/" if it is not add it to the string
        if(webURL.substr(-1) !== "/"){
            webURL = webURL + "/"
            console.log(webURL);
        }
        ...
        $("#pasteRedirects").css({"display": "none"});
        ...
        $("#submitRedirects").click(function () {
            $(".matchingRule").each(function (i, obj) {
                ...
                //remove the  button and .matchingRule
                $( ".matchingRule" ).hide();
                $( "#submitRedirects" ).hide();
                //add a save button 
        $(".container-header").after($('<button class="save-redirects btn green" data-ember-action=""<i aria-hidden=""class="fa fa-save ember-view"></i> Save Redirects</button>'));
            });
        });
